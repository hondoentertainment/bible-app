...
/** Higher score = better partial match (prefix and substring friendly). */
...
export function rankByPartialMatch<T>(
  items: T[],
  query: string,
  getLabel: (item: T) => string,
): T[] {
  return [...items].sort(
    (a, b) => scorePartialMatch(getLabel(b), query) - scorePartialMatch(getLabel(a), query),
  )
}

...
export function ReadingHistoryStrip({ onSelect }: ReadingHistoryStripProps) {
  ...
  return (
    <section className="mb-8" aria-label="Recently read verses">
      ...
      <div className="-mx-1 flex gap-2 overflow-x-auto px-1 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {entries.slice(0, 12).map((entry) => (
          <button
            ...
            title={entry.text}
            className="group flex min-w-[9rem] max-w-[14rem] shrink-0 flex-col rounded-xl border border-parchment-dark bg-white px-4 py-3 text-left shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:border-gold/50 hover:shadow-md active:scale-[0.98]"
          >
            <span className="font-display text-sm font-semibold text-navy group-hover:text-gold">
              {entry.reference}
            </span>
            {entry.text && (
              <span className="mt-1 line-clamp-2 text-xs leading-snug text-ink-muted">{entry.text}</span>
            )}
          </button>
        ))}
      </div>
